    ...
    const LocateControl = L.Control.extend({
        ...
        _activate: async function (defaultLatLng) {
            if (!this._active) {
                this._active = true;
                try {
                    if (!!defaultLatLng?.lat && !!defaultLatLng?.lng && !!defaultLatLng?.accuracy) {
                        this._setEvent(defaultLatLng.lat, defaultLatLng.lng, defaultLatLng.accuracy);
                    }
                    else {
                        const position = await Geolocation.getCurrentPosition({ enableHighAccuracy: true, timeout: 50000 });
                        this._setEvent(position.coords.latitude, position.coords.longitude, position.coords.accuracy);
                    }
                } catch (e) {
                    console.error("🚀 ~ error _activate:", e)
        ...
        _onLocationFound(e) {
            ...
            if (!this._event || !e?.coords) return;
            // no need to do anything if the location has not changed
            if (this._event &&
                (this._event.latlng.lat === e.coords.latitude &&
                    this._event.latlng.lng === e.coords.longitude)) {
                return;
            }
            ...
            this._setEvent(e.coords.latitude, e.coords.longitude, e.coords.accuracy);
        ...
        _setEvent(latitude, longitude, accuracy) {
            this._event = {
                "latlng": {
                    "lat": latitude,
                    "lng": longitude,
